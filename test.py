...
# Use the same connection parameters as your app
graph = Neo4jGraph(
    url="bolt://127.0.0.1:7687",
    username="neo4j",
    password="12345678",
    database="herplloveu"  # Make sure this is correct
)

# Test simple queries
try:
    # Test 1: Count all nodes
    result1 = graph.query("MATCH (p:Patient) RETURN count(p) LIMIT 5 ")
    print(f"Total nodes: {result1[0] if result1 else 'No result'}")
    
    # # Test 2: Get all labels
    # result2 = graph.query("CALL db.labels() YIELD label RETURN label")
    # print(f"Available labels: {[r['label'] for r in result2]}")
    
    # # Test 3: Try specific Doctor query
    # result3 = graph.query("MATCH (d:Doctor) RETURN d LIMIT 5")
    # print(f"Doctor query returned {len(result3)} results")
    
    # # Print first result with proper serialization
    # if result3:
    #     doctor = result3[0].get('d', {})
    #     serialized = json.dumps(doctor, default=str, indent=2)
    #     print(f"First doctor: {serialized}")
